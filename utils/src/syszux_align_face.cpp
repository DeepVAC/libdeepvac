...
AlignFace::AlignFace()
{
    ...
    ref_facial_5pts_ = cv::Mat(5, 2, CV_32FC1, &REFERENCE_FACIAL_POINTS_112x112);

    float CROP_SIZE[2] = {112,112};
    crop_size_ = cv::Mat(1,2,CV_32FC1,&CROP_SIZE);
}
...
cv::Mat AlignFace::warpAndCrop(cv::Mat& src_img, cv::Mat& facial_5pts)
{
    ...
    cv::Mat dst_img;
	cv::warpAffine(src_img, dst_img, tfm, cv::Size(crop_size_.at<int>(0), crop_size_.at<int>(1)));
